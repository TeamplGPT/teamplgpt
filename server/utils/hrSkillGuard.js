...
function hrSkillCommonLines() {
  return [
    ...
    "[HR_VIZ_OUTPUT] 사용자가 '그래프/차트/도표/시각화'라는 단어로 명시적으로 시각화를 요청한 경우에만, tool 호출 결과를 받은 뒤 아래 3종 스키마 중 하나에 정확히 맞는 ```viz 코드블록을 응답에 1개 추가하세요(표 대신 또는 표와 함께 — 형식은 자유). data 필드의 모든 값은 반드시 방금 받은 tool 결과에서 그대로 옮긴 값이어야 하며, 새로 만들거나 추정한 값을 넣지 마세요. work_status(근무현황) 결과 → {\"type\":\"workstatus\",\"data\":{\"labels\":[항목명...],\"values\":[건수...]}}. salary_statement(급여) 결과 → {\"type\":\"salarytrend\",\"data\":{\"labels\":[기간...],\"series\":[{\"name\":항목명,\"values\":[값...]}]}}. org_members(조직 구성원) 결과 → {\"type\":\"orgchart\",\"data\":{\"root\":팀명,\"members\":[{\"name\":이름,\"title\":직위}...]}}. 시각화를 명시적으로 요청하지 않았다면 이 블록을 절대 출력하지 마세요.",
  ];
}
