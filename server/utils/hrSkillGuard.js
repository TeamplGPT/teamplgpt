...
function hrSkillCommonLines() {
  return [
    ...
    "[HR_VIZ_OUTPUT] 사용자가 '그래프/차트/도표/시각화'라는 단어로 명시적으로 시각화를 요청했거나(work_status·salary_statement 결과), org_members 결과를 '조직도'로 보여달라고 요청한 경우에만(도(圖)=그림이므로 '조직도' 자체가 이미 시각화 요청입니다), tool 호출 결과를 받으면 [지시 순서]를 그대로 따르세요. [지시 순서] 1) 코드펜스 언어 태그는 반드시 정확히 소문자 viz 하나만 쓰세요(json·JSON·Viz 등 다른 표기 절대 금지) — ```viz로 시작해 ```로 닫습니다. 2) 그 안에 아래 3종 스키마 중 하나에 정확히 맞는 JSON을 1개만 채우세요. data 필드의 모든 값은 방금 받은 tool 결과에서 그대로 옮긴 값이어야 하며(구성원·항목을 누락·축약하지 말고 전부 포함), 새로 만들거나 추정한 값을 넣지 마세요. 3) viz 블록을 다 채운 뒤에만 짧은 안내 문장(1~2문장, 예: 'N명의 조직도를 표시했습니다')을 덧붙이세요 — 인원·항목을 텍스트로 다시 나열하거나 표로 반복하지 마세요(viz 블록과 중복 작성 금지, 특히 구성원 수가 많을 때 이 규칙이 중요합니다). work_status(근무현황) 결과 → {\"type\":\"workstatus\",\"data\":{\"labels\":[항목명...],\"values\":[건수...]}}. salary_statement(급여) 결과 → {\"type\":\"salarytrend\",\"data\":{\"labels\":[기간...],\"series\":[{\"name\":항목명,\"values\":[값...]}]}}. org_members(조직 구성원) 결과 → {\"type\":\"orgchart\",\"data\":{\"root\":방금 조회된 실제 조직/팀명,\"members\":[{\"name\":이름,\"title\":직위}...]}}(root를 임의로 지어내지 마세요). 위 트리거(그래프/차트/도표/시각화/조직도) 없이는 이 블록을 절대 출력하지 마세요.",
  ];
}
