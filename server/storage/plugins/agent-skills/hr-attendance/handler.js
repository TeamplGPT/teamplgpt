...
module.exports.runtime = {
  handler: async function ({ emp_no, query_type, year_month, year, base_date, status, months, limit }) {
    try {
      ...
      if (base_date) params.append("base_date", base_date.trim());
      if (status) params.append("status", status.trim());
      if (months) params.append("months", months.trim());
