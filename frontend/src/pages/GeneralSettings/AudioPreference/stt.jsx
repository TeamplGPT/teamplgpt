...
const PROVIDERS = [
  {
    ...
    description: "speech-text.speech.providers.native.description",
  },
];
...
export default function SpeechToTextProvider({ settings }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      showToast(t("speechText.speech.error", { error }), "error");
    } else {
      showToast(t("speech-text.speech.success"), "success");
    }
    setSaving(false);
    setHasChanges(!!error);
  };
        ...
        <div className="w-full flex flex-col gap-y-1 pb-6 border-white light:border-theme-sidebar-border border-b-2 border-opacity-10">
          <div className="flex gap-x-4 items-center">
            <p className="text-lg leading-6 font-bold text-white">
              {t("speech-text.speech.title")}
            </p>
          </div>
          <p className="text-xs leading-[18px] font-base text-white text-opacity-60">
            {t("speech-text.speech.description")}
          </p>
        </div>
              ...
              <div className="w-full flex flex-col gap-y-1">
                <div className="flex items-center sticky top-0 z-10 border-b border-[#9CA3AF] mx-4 bg-theme-settings-input-bg">
                  ...
                  <input
                    ...
                    autoComplete="off"
                    placeholder={t("speech-text.speech.search")}
