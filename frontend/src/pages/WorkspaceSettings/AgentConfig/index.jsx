...
export default function WorkspaceAgentConfiguration({ workspace }) {
  ...
  const handleUpdate = async (e) => {
    ...
    if (!!updatedWorkspace) {
      showToast(t("agent.update-success"), "success", {
        clear: true,
      });
    } else {
    ...
    <div id="workspace-agent-settings-container">
      ...
      >
          ...
          <>
            {!hasChanges && (
              <div className="flex flex-col gap-y-4">
                ...
                >
                  {t("agent.configureAgentSkills")}
                </a>
                <p className="text-white text-opacity-60 text-xs font-medium">
                  {t("agent.configureAgentSkillsDescription")}
                </p>
              </div>
            )}
          </>
        ...
        {hasChanges && (
          ...
          >
            {saving
              ? t("agent.updatingAgent")
              : t("agent.updateWorkspaceAgent")}
          </button>
        )}
      </form>
    </div>
