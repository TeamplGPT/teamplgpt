...
export default function SystemPromptVariables() {
  ...
  const fetchVariables = async () => {
    ...
    try {
      ...
      showToast(t("system_prompt_variables.no-variables"), "error");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex flex-col w-full px-1 md:pl-6 md:pr-[50px] md:py-6 py-16">
          <div className="w-full flex flex-col gap-y-1 pb-6 border-white/10 border-b-2">
            <div className="items-center flex gap-x-4">
              <p className="text-lg leading-6 font-bold text-theme-text-primary">
                {t("system_prompt_variables.title")}
              </p>
            </div>
            <p className="text-xs leading-[18px] font-base text-theme-text-secondary">
              {t("system_prompt_variables.description")}
            </p>
          </div>
          ...
          <div className="w-full justify-end flex">
            ...
            >
              ...
              {t("system_prompt_variables.add.button")}
            </CTAButton>
          </div>
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <div className="text-center py-4 text-theme-text-secondary">
                {t("system_prompt_variables.no-variables")}
              </div>
            ) : (
              <table className="w-full text-sm text-left rounded-lg min-w-[640px] border-spacing-0">
                <thead className="text-theme-text-secondary text-xs leading-[18px] font-bold uppercase border-white/10 border-b">
                  <tr>
                    ...
                    <th scope="col" className="px-4 py-2">
                      {t("system_prompt_variables.table.type")}
                    </th>
                  </tr>
                </thead>
                <tbody>
