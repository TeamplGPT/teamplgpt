...
export default function VariableRow({ variable, onRefresh }) {
  ...
  const handleDelete = async () => {
    ...
    if (
      !window.confirm(
        t("system_prompt_variables.table.delete_confirm", { key: variable.key })
      )
    )
      return false;
    ...
    try {
      ...
      showToast(t("system_prompt_variables.table.delete_success"), "success", {
        clear: true,
      });
      if (onRefresh) onRefresh();
    } catch (error) {
      console.error("Error deleting variable:", error);
      showToast(t("system_prompt_variables.table.delete_failed"), "error", {
        clear: true,
      });
    }
  };
        ...
        <td className="px-4 py-2 flex items-center justify-end gap-x-4">
          {variable.type === "static" && (
            <>
              ...
              >
                {t("system_prompt_variables.table.edit")}
              </button>
              <button
