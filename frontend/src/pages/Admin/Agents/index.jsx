...
export default function AdminAgents() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      ...
      setImportedSkills(_preferences.settings?.imported_agent_skills ?? []);
      showToast(t("agent.skill.agentPreferencesSavedSuccessfully"), "success", {
        clear: true,
      });
    } else {
      showToast(t("agent.skill.agentPreferencesFailedToSave"), "error", {
        clear: true,
      });
    }
            ...
            <div className="space-y-4">
              ...
              <div className="text-theme-text-primary flex items-center justify-between gap-x-2 mt-4">
                ...
                {agentFlows.length === 0 ? (
                  ...
                  >
                    <Hammer size={16} />
                    <p className="text-sm">{t("agent.agent-flows.create")}</p>
                  </Link>
                ) : (
                  <Link
                    to={paths.agents.builder()}
                  ...
                  >
                    <Hammer size={16} />
                    <p className="text-sm">
                      {t("agent.agent-flows.openBuilder")}
                    </p>
                  </Link>
                )}
              </div>
        ...
        <div className="flex-[2] flex flex-col gap-y-[18px] mt-10">
          <div className="bg-theme-bg-secondary text-white rounded-xl flex-1 p-4 overflow-y-scroll no-scroll">
            {SelectedSkillComponent ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-theme-text-secondary">
                <Robot size={40} />
                <p className="font-medium">{t("agent.skill.description")}</p>
              </div>
            )}
          </div>
        </div>
