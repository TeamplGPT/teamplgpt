...
const TRANSLATIONS = {
  ...
  agent: {
    ...
    skill: {
      ...
      "sql-agent": {
        ...
        port: "포트 번호",
        database: "데이터베이스",
        database_description: "에이전트가 상호작용할 데이터베이스",
        encryption: "암호화 활성",
    ...
    "agent-flows": {
      ...
      "delete-flow-confirm":
        "이 플로우를 정말 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.",
      "flow-deleted-successfully": "플로우가 성공적으로 삭제되었습니다.",
      "failed-to-delete-flow": "플로우 삭제에 실패했습니다.",
      editflow: "플로우 수정",
      deleteflow: "플로우 삭제",
      flowInformation: {
        title: "플로우 정보",
  ...
  recorded: {
    ...
    chatrow: {
      ...
      "export-success": "채팅이 {{format}} 형식으로 성공적으로 내보내졌습니다.",
      "clear-all-confirm":
        "모든 채팅을 정말로 삭제하시겠습니까?\n\n이 작업은 되돌릴 수 없습니다.",
  ...
  transcription: {
    ...
    description:
      "선호하는 텍스트 변환 모델 제공자의 인증입니다. 이 키가 현재 활성 상태이고 정확해야 미디어 파일 및 오디오가 텍스트로 변환됩니다.",
    provider: "텍스트 변환 제공자",
    providers: {
      openai: {
        description:
  ...
  embedding: {
    ...
    "desc-end":
      "임베딩은 텍스트를 벡터로 변환하는 과정입니다. 파일과 프롬프트를 AnythingLLM이 처리할 수 있는 형식으로 변환하려면 인증이 필요합니다.",
    messages: {
  ...
  vector: {
    ...
    switchWarning: {
      title: "경고 - 이 작업은 되돌릴 수 없습니다",
      message:
        "벡터 데이터베이스를 변경하면 모든 워크스페이스에서 이전에 임베딩된 모든 문서가 초기화됩니다.\n\n확인하면 벡터 데이터베이스의 모든 임베딩이 삭제되고 워크스페이스의 모든 문서가 제거됩니다. 업로드된 문서는 삭제되지 않으며, 재임베딩이 가능합니다.",
      confirm: "확인",
      cancel: "취소",
      proceed: "계속 진행하시겠습니까?",
    },
    messages: {
      ...
      saveError: "벡터 데이터베이스 설정 저장에 실패했습니다: {{error}}",
    },
    provider: {
      title: "벡터 데이터베이스 제공자",
