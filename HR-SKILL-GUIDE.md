...
- 이 문서는 "어디를 보고, 어떤 순서로, 무엇을 검증해야 하는가"의 지도다. 세부 규칙 정본은 각 절에 링크된 문서.

---

## 0. 사전 준비 — 3개 프로젝트 clone + 로컬 경로 등록

로컬 경로는 개인별로 다르므로 이 문서는 **경로 변수**로 표기한다. 먼저 3개 리포를 clone:
...
**AI Agent(Claude Code 등)와 작업할 때** — 에이전트는 다른 프로젝트의 로컬 위치를 모른다. 세션 시작 시 한 번 등록:

1. 세션 첫 메시지에 세 경로를 알려주거나:
   > "kiwibox는 `~/work/kiwibox_eGov4.2`, okrservice는 `~/work/okrservice`에 clone돼 있음"
2. 또는 영구 등록 — 개인 전역 `~/.claude/CLAUDE.md`(리포에 커밋되지 않음)에:
   ```markdown
   # HR 프로젝트 로컬 경로
   ...
   ```
3. 경로를 모를 때 에이전트에게 시킬 탐색 명령(에이전트가 직접 실행 가능):
   ```bash
...
⚠️ **공유 산출물 규칙**: 팀에 공유되는 에이전트 작업 결과물(spec·analysis·작업지시서·이 가이드 등)에는 `/home/<username>/...` 절대경로를 쓰지 말 것. 리포 내부는 리포 상대경로, 타 리포는 `$KIWIBOX/...` 식 변수 표기. 기존 문서에서 절대경로를 발견하면 보정 대상.

---

## 1. 시스템 지형 — 3개 프로젝트의 역할
...
**두 가지 실행 모드** (`server/storage/plugins/agent-skills/_shared/hrSession.js`가 자동 선택):
...
**동기화 규칙 (중요)**: teamplgpt에서 **endpoint 경로/cmd를 바꾸면 okrservice hrBridge의 `ALLOWED_PATHS`(정적 allowlist)가 자동으로 안 바뀐다** → 브리지가 `"bridge: path not allowed"`로 즉시 차단. 반드시:
...
| `{skill}/handler.js` | **L2 코드 계층**: ENDPOINT_MAP(경로·cmd·BODY 고정값·`columns` 화이트리스트) + 렌더. **handler.js 무수정 원칙 = LLM 행태 제어를 handler에 넣지 말 것**(description으로). 코드 가드(화이트리스트·검증)는 handler 소관 |
...
⚠️ 두 가드는 **미러 관계** — 한쪽만 고치면 경로별 행태가 갈라진다(실사고: `[HR_TABLE_OUTPUT]` 구 문구가 footer와 충돌해 embed에서만 표 통짜 출력). 문구 수정 시 양쪽 + footer-contract 대조.

### 3.3 채팅 4면 (수정 시 전부 확인)

`chat/query` · `react` · `@agent` · **embed** — 상세: `docs/rag-search-flow-chat-vs-react.md`. embed 전용 배선: `server/utils/chats/embed.js`(tool_choice=required 강제, allowlist 필터, `clientToolBroker` R1 주입).

---

## 4. Multi-Layer Defense — 완료 판정 기준

한 층만 고치고 "완료" 보고 금지. 세 층 모두:
...
핵심 원칙: **LLM 프롬프트(L1·가드)는 확률적** — 민감정보 차단은 반드시 L2(tool 결과에서 원천 제거)로. L1/footer는 품질(발췌·요약·되묻기 억제) 담당.
...
- **`docs/conventions/hr-skill-description-pattern.md`** — description 작성 표준. 작업 종류별 필요 섹션 매트릭스가 CLAUDE.md에 있음. 핵심: 주기 파라미터는 `[CRITICAL]` 3단 + `[재강조]` 필수(Template T-A/T-B), 신규 query_type은 **3-Location 패턴**(Location A: skill description 매핑표 / B: query_type param description / C: examples).

### 5.2 신규 query_type(또는 endpoint 교체) 추가 — 표준 순서

1. **kiwibox 정본 확인**: `cmmAiAssistantToolEndpoints.md`에서 경로·cmd·필수 BODY·범위(a=세션 신원 강제/b=cmmSearchStaffId)·민감 경고 확인. 카탈로그 §7 등록 금지 endpoint(주민번호 반환 등)는 절대 연동 금지.
2. **응답 컬럼 대조**: sqlmap `*_SQL.xml`에서 최외곽 SELECT 별칭 전수 추출 → spec-db `Table/*.md`·화면 JSP grid로 한글 라벨 확정 → **화이트리스트 설계**(내부 식별자·`*_CD` 코드값·복호화 컬럼 차단). 판정 선례: `docs/03-analysis/hr-column-whitelist-audit.analysis.md`.
   - egovMap이면 응답 키는 camelCase(`STAFF_TYPE_NM`→`staffTypeNm`). alias 중복 시 **뒤 컬럼이 이김**(실사고: SALDaylab `SAL_CLASS_NM` 호봉명↔은행명 오염).
3. **실호출 검증**: `specs/kiwibox-endpoint-test-guide.md` 절차로 ntest.5240.kr에 curl — 필수 파라미터 누락 시 빈 응답이 흔하므로 실측 BODY 전량 원칙(임의 축약 금지, `searchType=mobile` 금지).
4. **E2E 시나리오 먼저** (E2E-First): `scenarios.json`에 tool_call + `mock_url_pattern` + `mock_body_pattern`(+ 필요시 `answer_pattern`/`answer_not_pattern`/`max_hr_calls`) append → **FAIL 확인**.
5. mock fixture 추가: `mock-hr-api.js` `FIXTURES_BY_CMD`(cmd 기반) 또는 `FIXTURES_BY_PATH`(cmd 없는 경로형). **실측 키 그대로**(실사고: work_status식 키로 넣어 timesheet 렌더 공란).
...
트랙 5 스펙 경로 필수. convention §3 T-B/§4 T-A 템플릿 + §7 3-Location. 기존 skill(`hr-attendance`가 가장 표준적) 복제 후: ENDPOINT_MAP·QUERY_LABELS·COLUMNS_BY_QT·plugin.json 전면 재작성. `_shared`는 수정 없이 재사용이 원칙(수정 시 전 skill 회귀).
...
**사전 조건**: AnythingLLM 서버 `:3001` 기동(`yarn dev:server` 또는 `yarn dev:all`) + docker `anythingllm-postgres` healthy + workspace slug `eshelsoft` + HR skill 활성. LLM은 실제 provider 호출(결정론 아님 — 시나리오는 관대한 정규식으로).
...
### 6.2 코드 반영 확인 (자주 틀리는 부분)

- `handler.js`는 호출마다 require 캐시 삭제 → **서버 재기동 없이 반영**.
- `_shared/*.js`·`server/utils/**`는 **서버 재기동 필요**(require 캐시 잔존/nodemon 범위 밖일 수 있음). E2E 돌리기 전 재기동이 안전.
- plugin.json description 변경도 재기동 권장(로드 시점 캐시).

### 6.3 실환경 검증

- kiwibox 직접: `specs/kiwibox-endpoint-test-guide.md` (로그인 → 쿠키 → curl per endpoint).
- 위젯 경유(R1): okrservice 로컬 기동 + embed 설정(§7) 후 실브라우저. 최종 판정은 **새 세션**에서(기존 대화 히스토리가 과거 잘못된 답변 패턴을 모방시킬 수 있음).
- LLM 답변 행태 변경은 반드시 4면 확인: chat/query·react·@agent·embed.

### 6.4 관측성

- `HR_DEBUG_TOOL_IO=true` → `[tool-io]` 한 줄 JSON 로그(LLM 입출력·TOOL_CALL·KIWIBOX_RAW). `grep tool-io`로 실데이터 추적. ⚠️ 급여·개인정보 포함 가능 — 운영에서 상시 금지.

---

## 7. okrservice 연동 계약 (운영 배선)

정본: okrservice `docs/teamplgpt-hr-client-tools-workorder.md`. 요점:
...
| timesheet 출근/퇴근 컬럼 공란 | mock fixture 키를 실측(`staTime`)이 아닌 추정(`inTime`)으로 작성 | fixture는 sqlmap 별칭 기준. 실측 42필드는 timesheet·work_status 키 **둘 다** 포함(동일 endpoint 공유) |
