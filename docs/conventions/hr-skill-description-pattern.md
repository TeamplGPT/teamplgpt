...
## §6.3 파라미터 계약의 기본형 — "생략 가능 + 서버가 해석"

**self 컨텍스트나 자료 상태로 결정될 수 있는 파라미터는 전부 선택(optional)이고, 생략하면 서버가
해석한다.** 이걸 어기고 "필수"로 두면 LLM이 값을 **지어낸다.**

이미 이 모양인 것들:

| 파라미터 | 생략 시 서버 해석 | 근거 |
|---|---|---|
| `year_month` | 이번 달. 결재 미결함은 전체 기간, 급여는 최근 지급건 | `_shared/periodPolicy.js` |
| `cal_yy` | 지원 연도를 순회해 **자료가 있는 해** | `_shared/selfContext.js` `firstNonEmpty` |
| `pay_item` | 최근 지급 건 | `hr-salary` `fetchRecentPayPeriods` |
| `org_cd` | **본인 소속 조직** | `_shared/selfContext.js` `resolveSelfOrgCd` |

`org_cd`만 "org_members에서 **필수**, org_tree 결과값만 사용"으로 남아 있었고, 그래서
"우리 팀원 누구야?"에 모델이 조직 147개 중 하나를 골라 넣어 **엉뚱한 팀**을 조회했다
(2026-08-21 실측: 본인은 인사팀 0303인데 0111 조립설계팀). 조회가 성공해 사용자가
오답을 알아채기 어렵다는 점에서 "답변 불가"보다 나쁘다.

### 신규 파라미터 판정

- [ ] 이 값을 **서버가 알 수 있나?** (세션 신원·직전 조회 결과·자료 존재 여부)
      → 알 수 있으면 **선택**으로 두고 서버가 채운다. 필수로 두지 않는다.
- [ ] 사용자가 **명시적으로 다른 대상을 지목**할 때만 LLM이 값을 넣도록 문구를 쓴다.
      "생략하면 X가 적용됩니다 / 특정 Y를 지목했을 때만 전달하세요" 형태.
- [ ] `[재강조]`에 **추측 금지**를 넣는다 — 값이 유효하기만 하면 조회가 성공해 버려서
      틀린 값이 걸러지지 않는다(존재 검증으로는 못 막는다).
- [ ] 서버 해석 로직은 handler에 흩지 말고 `_shared`에 둔다.

> 사번(`$SELF_STAFF_ID` 마커)이 이 원칙의 원형이다 — LLM에게 아예 노출하지 않고 서버가 채운다.
> 노출해야 하는 파라미터라면 최소한 **기본값을 서버가 갖는다.**
